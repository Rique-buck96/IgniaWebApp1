...
namespace RxWebApp.Data
{
    internal sealed class DiscussionRepository : RepositoryBase, IDiscussionRepository
    {
        ...
        public IObservable<DiscussionEntity> CreateDiscussion(int discussionId, string subject, string location, string employee, string outcome, DateTime discussionDate)
        {
            //CreateOrder(customerId, null);
            return CreateDiscussion(discussionId, subject, location, employee, outcome, discussionDate, null);
        }

        public IObservable<DiscussionEntity> CreateDiscussion(int discussionId, string subject, string location, string employee, string outcome, DateTime discussionDate, IScheduler scheduler)
        {
            var order = new DiscussionEntity { /*DiscussionId = discussionId,*/ Id = _discussionIdCounter++, Sub = subject, Loc = location, Emp = employee, Out = outcome};
